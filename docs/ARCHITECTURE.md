...
| Test cohorts | Selected frozen datasets, conditions and standalone target → immutable test membership | List → Test Data → Prediction Targets → Review and Freeze. Cohort validity is independent of models and features. Legacy bound records remain readable. |
| Evaluate models | Development predictors, test cohort and reviewed execution inputs → predictions, metrics and paired ensemble/refit comparisons | Review validates target encoding, development overlap, representation and exact feature/pack coverage. Saved plans pin resolved features. Unlabeled inference is possible; metrics use labeled records. |
