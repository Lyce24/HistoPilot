// Build an offline file:// fixture containing the real project-settings editor.
...
await build({
  ...
  build: {
    ...
    lib: { entry: resolve(root, 'web/verification/SettingsFixture.tsx'), formats: ['iife'], name: 'SettingsReview', fileName: () => 'settings.js' },
    cssCodeSplit: false,
  },
...
const js = await readFile(resolve(output, 'assets/settings.js'), 'utf8');
await writeFile(resolve(output, 'index.html'), `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Offline settings verification</title><style>${css}</style></head><body><div id="app"></div><script>${js.replaceAll('</script', '<\\/script')}</script></body></html>`);
