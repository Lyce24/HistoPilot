...
const output = resolve(process.argv[2] ?? '/tmp/histopilot-evaluation-review/browser');
await build({
  configFile: false, root: resolve(repository, 'web'), plugins: [react(), tailwindcss()],
  build: {
    outDir: resolve(output, 'assets'), emptyOutDir: true,
    lib: { entry: resolve(repository, 'web/verification/EvaluationFixture.tsx'), formats: ['iife'], name: 'EvaluationReviewHarness', fileName: () => 'review.js' },
...
await writeFile(resolve(output, 'index.html'), `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>HistoPilot evaluation offline verification</title><style>${css}</style></head><body><div id="app"></div><script>${inlineJs.replaceAll('</script', '<\\/script')}</script></body></html>`);
