...
export const ROADMAP_MODULES: readonly RoadmapModuleDefinition[] = [
  ...
  },
  {
    ...
    description: 'Select development data, define the target, and freeze training, validation and assessment memberships.',
    prerequisites: ['dataset'],
  },
  {
    id: 'features', title: 'Slide features', shortTitle: 'Slide features', phase: 'prepare',
    description: 'Register or extract slide features, check coverage, and save a verified bundle.',
    prerequisites: ['dataset'],
  },
  {
  ...
  },
  {
    ...
    description: 'Overlay ABMIL attention from a refit or ensemble predictor on slides and inspect influential regions.',
    prerequisites: ['clinical-utility'],
  },
];
...
export function protocolBundleCompatible(protocol: Configuration, bundle: FeatureBundle): boolean {
  ...
  return protocol.manifest.datasetId === bundle.manifest.datasetId
...
function bundleReady(bundle: FeatureBundle, datasetIds: Set<string>): boolean {
  return bundle.current
    && datasetIds.has(bundle.manifest.datasetId)
...
export function buildRoadmap(workspace: Workspace, evidence: Partial<RoadmapEvidence> = {}): RoadmapModule[] {
  ...
  const readyBundles = saved.bundles.filter((bundle) => bundleReady(bundle, datasetIds));
