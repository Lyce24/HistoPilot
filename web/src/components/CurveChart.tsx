...
export default function CurveChart({ title, description, xLabel, yLabel, series, xRange = [0, 1], yRange, referenceDiagonal = false }: {
  title: string; description: string; xLabel: string; yLabel: string; series: CurveSeries[]; xRange?: Range; yRange?: Range; referenceDiagonal?: boolean;
}) {
  const id = useId();
  ...
  return <section className="evidence-chart" aria-labelledby={`${id}-title`}>
    ...
    {available ? <><svg viewBox="0 0 520 300" role="img" aria-labelledby={`${id}-title ${id}-desc`}>
      ...
      {[0, .25, .5, .75, 1].map((fraction) => { const y = yr[0] + fraction * (yr[1] - yr[0]); const x = xr[0] + fraction * (xr[1] - xr[0]); return <g key={fraction}><line x1="65" x2="495" y1={py(y)} y2={py(y)} className="evidence-grid" /><text x="58" y={py(y) + 4} textAnchor="end">{formatStatistic(y, 2)}</text><text x={px(x)} y="263" textAnchor="middle">{formatStatistic(x, 2)}</text></g>; })}
