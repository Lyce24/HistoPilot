type Step = 'post-development' | 'evaluation' | 'clinical-utility' | 'interpretation';
...
export default function EvidenceChain({ current, ...context }: EvidenceContext & { current: Step }) {
  return <nav className="chain-banner" aria-label="Model evidence workflow">{([
    ['experiments', 'Experiments'], ['post-development', 'Predictors'], ['evaluation', 'Model evaluation'],
    ['clinical-utility', 'Clinical utility'], ['interpretation', 'Model interpretation'],
  ] as const).map(([module, label], index) => <span className="evidence-chain-step" key={module}>{index ? <span aria-hidden="true">→</span> : null}{current === module ? <strong aria-current="step">{label}</strong> : <a href={evidenceLink(module, context)}>{label}</a>}</span>)}</nav>;
}
