...
export default function FeatureBundlePreparation({ project, configuration, configurations, initialPackIds = [], onSelectVersion, onFrozen }: {
  project: string;
  ...
  function changePacks(ids: string[]) {
    ...
    setReview(null); setError(null); setNaming(false);
    operation.current = '';
  }
  async function preview() {
    const token = generation.current;
    setBusy(true); setError(null); setReview(null); setNaming(false);
    operation.current = '';
    try {
  ...
  return <div className="stack">
    <FeaturePacking project={project} configuration={configuration} configurations={configurations} onSelectVersion={onSelectVersion} selectedPackIds={packIds} onSelectedPackIdsChange={changePacks} />
    <section className="feature-bundle-freeze" aria-label="Freeze feature bundle">
      <div><h3>Freeze this bundle</h3><p>Save the feature source and the packs included with it as one immutable input. A features-only bundle includes no pack.</p></div>
      ...
      <button type="button" className="btn btn-primary science-fit" disabled={busy} onClick={() => void preview()}>{busy ? 'Checking bundle…' : 'Review bundle'} <Icon name="arrow" /></button>
    </section>
    {review ? <section ref={reviewRef} className="feature-bundle-review stack" aria-label="Bundle review">
    ...
    {naming && review?.canFreeze ? <FreezeVersionDialog kind="bundle" initialLabel={label} onLabelChange={setLabel} onClose={() => setNaming(false)} onFreeze={async (name) => {
      try {
        ...
        if (bundleReviewInvalidated(reason)) {
          generation.current += 1;
          operation.current = '';
          setReview(null); setNaming(false);
