...
const choices: { method: ExperimentPredictorPolicy['method']; title: string; description: string }[] = [
  ...
  { method: 'both', title: 'Ensemble + refit', description: 'Create both methods to compare on the same test cohort. Adds refit training.' },
  { method: 'skip', title: 'Cross-validation only', description: 'Finish with development results. No predictor is created for test-cohort evaluation.' },
];
  ...
  return <section className="batch-editor-section batch-predictor-settings" aria-label="Configure predictors">
    ...
    <fieldset className="experiment-predictor-fields"><legend className="sr-only">Predictor choices for this batch</legend>
      ...
      {refit ? <div className="experiment-refit-budget">
        ...
        <p>Each refit uses the selected percentile of its folds’ selected checkpoint epochs, rounded up. Checkpoints follow the batch’s evaluation policy. It trains on all development data for that budget, with early stopping disabled. Test cohorts stay out of training.</p>
      </div> : null}
    </fieldset>
