...
import { experiments, experimentPollInterval, experimentStatusLabel } from '../api/experiments';
import type { ExperimentInput, ModelExperiment } from '../api/experiments';
...
export function filterExperiments<T extends Pick<ModelExperiment, 'id' | 'name' | 'notes' | 'tags' | 'state' | 'status' | 'createdAt' | 'updatedAt'>>(items: T[], state: LifecycleState | 'all', status: string, search: string, sort: string) {
  const query = search.trim().toLocaleLowerCase();
  return items.filter((item) => (state === 'all' || item.state === state) && (!status || item.status === status)
...
export function ExperimentComparison({ items }: { items: ModelExperiment[] }) {
  ...
  const chosenBatch = (item: ModelExperiment) => batchIds[item.id] ?? '';
  const rows = experimentDifferences(ordered.map((item) => comparisonSnapshot(item, chosenBatch(item))), differencesOnly);
  return <Panel title="Compare experiment inputs" subtitle="Compare saved inputs or select an immutable batch snapshot for each experiment. Training differences include every saved recipe field, configuration, seed and resource setting.">
...
export function CreateExperiment({ project, copy, onCreated, onClose }: { project: string; copy?: ModelExperiment; onCreated: (item: ModelExperiment) => void; onClose: () => void }) {
  ...
  const [pending, setPending] = useState<(ExperimentInput & { operationId: string }) | null>(null);
  async function create() {
    setBusy(true); setError(null);
    const input = pending ?? { name: name.trim(), notes: notes.trim(), tags: [...new Set(tags.split(',').map((value) => value.trim()).filter(Boolean))], ...(copy?.inputs ? { inputs: copy.inputs } : {}), operationId: crypto.randomUUID() };
  ...
  return <Panel title={copy ? 'Create experiment from saved inputs' : 'Create experiment'} subtitle="Create a named record first. Add inputs and training batches inside it; training starts only when you launch a batch.">
    <form onSubmit={(event) => { event.preventDefault(); void create(); }}><fieldset disabled={busy || Boolean(pending)} className="experiment-create-fields"><legend className="sr-only">Experiment details</legend>
      ...
      <label className="label">Notes<textarea className="field" value={notes} maxLength={2000} onChange={(event) => setNotes(event.target.value)} placeholder="What are you testing in this experiment?" /></label>
    </fieldset>{copy ? <p className="muted">Only saved inputs are copied. The source experiment, batches, jobs, and predictor keep their existing identities.</p> : null}
...
export function ExperimentMetadata({ project, record }: { project: string; record: ModelExperiment }) {
  ...
  async function save() {
    ...
    try {
      const saved = await experiments.update(project, record.id, { name: name.trim(), notes, tags: [...new Set(tags.split(',').map((value) => value.trim()).filter(Boolean))], inputs: record.inputs, expectedRevision: revision });
...
export default function ExperimentRegistry({ project, onOpen }: { project: string; onOpen: (id: string) => void }) {
  ...
  return <div className="clinical-workspace experiment-registry">
    ...
    <div className="experiment-flow"><span><strong>Experiments</strong><small>Inputs · batches · runs · results</small></span><span aria-hidden="true">→</span><a href="#post-development"><strong>Predictors</strong><small>Ensemble and refit models</small></a><span aria-hidden="true">→</span><a href="#evaluation"><strong>Evaluations</strong><small>Selected predictor + test cohort</small></a></div>
    {creating ? <CreateExperiment project={project} onClose={() => setCreating(false)} onCreated={(item) => { client.setQueryData(['model-experiment', project, item.id], item); void client.invalidateQueries({ queryKey: ['model-experiments', project] }); onOpen(item.id); }} /> : null}
    <ErrorNotice error={query.error} />
    <section className="card experiment-list" aria-label="Experiments">
      <div className="experiment-list-heading"><div><h2>Experiments</h2><p className="muted">{items.length} records · {items.filter((item) => ['running', 'queued', 'cancelling'].includes(item.status)).length} experiments with active jobs</p></div><button className="btn btn-secondary btn-small" onClick={() => void query.refetch()}>Refresh</button></div>
      ...
      <div className="experiment-filters"><label className="label">Find experiments<input type="search" className="field" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Name, ID, notes or tag" /></label><label className="label">Status<select className="field" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All statuses</option>{[...new Set(items.map((item) => item.status))].sort().map((value) => <option key={value} value={value}>{experimentStatusLabel(value)}</option>)}</select></label><label className="label">Sort<select className="field" value={sort} onChange={(event) => setSort(event.target.value)}><option value="recent">Recently updated</option><option value="oldest">Created first</option><option value="name">Name</option></select></label></div>
      ...
      {query.isPending ? <p className="panel-body" role="status">Loading experiments…</p> : visible.length ? <div className="experiment-table-scroll"><table className="experiment-record-table"><thead><tr><th scope="col"><span className="sr-only">Compare</span></th><th scope="col">Experiment</th><th scope="col">Status</th><th scope="col">Inputs</th><th scope="col">Batches / runs</th><th scope="col">Predictor</th><th scope="col">Updated</th><th scope="col">Actions</th></tr></thead><tbody>{visible.map((item) => <tr key={item.id}>
        ...
        <td><Badge tone={['running', 'queued', 'cancelling'].includes(item.status) ? 'warning' : item.status === 'completed' ? 'success' : 'neutral'}>{experimentStatusLabel(item.status)}</Badge>{item.state !== 'active' ? <small>{lifecycleLabel[item.state]}</small> : null}</td>
        ...
        <td>{item.batches.length} batches<small>{item.batches.reduce((total, batch) => total + batch.manifest.summary.runCount, 0)} planned runs</small></td>
        <td>{item.predictorId ? <a href={`#post-development?experiment=${encodeURIComponent(item.id)}`}>View predictors</a> : <span className="muted">Not built</span>}</td>
      ...
      </tr>)}</tbody></table></div> : <div className="experiment-empty"><h3>{items.length ? 'No experiments match this view' : 'Create your first experiment'}</h3><p>{items.length ? 'Adjust the status, search or archive filters to find earlier work.' : 'Start with a name and a question. Add prepared targets, features and training batches after creating the record.'}</p></div>}
    </section>
    ...
    <p className="muted">Archive keeps experiments available to existing predictors. Delete moves records to recoverable Trash; dependencies and running jobs are checked before confirmation. Files stay on disk.</p>
  </div>;
}
