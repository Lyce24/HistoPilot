...
import { StageBackButton, StageContinueButton } from '../components/StageWorkflow';
...
import { supportsAttention } from '../lib/modelCapabilities';
...
function InterpretationWorkspace({ workspace }: { workspace: Workspace }) {
  ...
  const contextSignature = JSON.stringify(['predictor', 'evaluation', 'clinical', 'search'].map((key) => parameters.get(key) ?? ''));
  ...
  useEffect(() => {
    ...
    if ((parameters.get('predictor') ?? '') !== draft.predictorId || (parameters.get('evaluation') ?? '') !== draft.evaluationId || (parameters.get('clinical') ?? '') !== draft.clinicalId || (parameters.has('search') && parameters.get('search') !== draft.search)) setDraft(restoreWizardDraft(project, parameters));
  }, [contextSignature, parameters, project, draft.predictorId, draft.evaluationId, draft.clinicalId, draft.search, draft.batch?.pending]);
  ...
  const ready = Boolean(predictor && supportsAttention(predictor.manifest.recipe.model, predictor.manifest.recipe.inputMode) && sourceMatches && representationMatches && source?.slideFolder && contextValid && !registry.isError && !sources.isError);
  ...
  if (stage === 'viewer') return <div className="clinical-workspace model-chains clinical-insights interpretation-wizard interpretation-wizard-viewer">
    <PageHeader eyebrow="MODEL INTERPRETATION" title={`Attention ${activeSlide?.slideId ?? ''}`.trim()} description={activeRecord ? `${predictorMethodLabel(activeRecord.manifest.method)} · ${activeRecord.manifest.name}` : 'Loading the selected slide.'} actions={<StageBackButton onClick={() => navigate('results')}>Back to results</StageBackButton>} />
  ...
  if (stage === 'results') return <div className="clinical-workspace model-chains clinical-insights interpretation-wizard">
    <PageHeader eyebrow="MODEL INTERPRETATION" title="Attention results" description="Choose one of your selected slides to open its attention viewer." actions={<StageBackButton onClick={() => navigate('select')}>Back to slide selection</StageBackButton>} />
  ...
  if (stage === 'compute') return <div className="clinical-workspace model-chains clinical-insights interpretation-wizard">
    <PageHeader eyebrow="MODEL INTERPRETATION" title="Prepare attention" description="Completed studies are skipped, active jobs are reused, and only missing attention is computed." actions={<StageBackButton onClick={() => navigate('select')}>Back to slide selection</StageBackButton>} />
  ...
  return <div className="clinical-workspace model-chains clinical-insights interpretation-wizard">
    <PageHeader eyebrow="OPTIONAL ANALYSIS" title="Model interpretation" description="Choose a model and shared features, then select slides. Continue to compute missing attention and review your results." />
    ...
    <Panel title="Choose model and shared features" subtitle="Slides are loaded automatically from the dataset attached to the feature bundle.">
      <fieldset className="interpretation-sources" disabled={batch.locked}><legend className="sr-only">Model and shared features</legend>
        <label className="label">Predictor<select className="field" value={draft.predictorId} onChange={(event) => changeSource({ predictorId: event.target.value, bundleId: null, packChoice: null, evaluationId: '', clinicalId: '' })}><option value="">Choose a frozen ABMIL or nnMIL predictor</option>{draft.predictorId && !predictor ? <option value={draft.predictorId} disabled>Linked predictor unavailable</option> : null}{(registry.data?.items ?? []).filter((item) => item.lifecycleState === 'active' || item.id === draft.predictorId && item.lifecycleState === 'archived').map((item) => <option key={item.id} value={item.id} disabled={!supportsAttention(item.manifest.recipe.model, item.manifest.recipe.inputMode)}>{item.manifest.name} · {predictorMethodLabel(item.manifest.method)}{!supportsAttention(item.manifest.recipe.model, item.manifest.recipe.inputMode) ? ' · attention unsupported' : ''}</option>)}</select></label>
...
export function GalleryWorkspace({ project, source, selected, search, offset, locked, canContinue, onSelection, onSearch, onPage, onContinue }: { project: string; source: GallerySource; selected: GallerySlide[]; search: string; offset: number; locked: boolean; canContinue: boolean; onSelection: (slides: GallerySlide[]) => void; onSearch: (search: string) => void; onPage: (offset: number) => void; onContinue: () => void }) {
  ...
  return <Panel title="Choose slides" subtitle="Click thumbnails or checkboxes to select slides. No computation starts until you continue.">
    <div className="slide-gallery-tools"><label className="label">Search slides<input type="search" className="field" maxLength={200} value={search} onChange={(event) => onSearch(event.target.value)} placeholder="Search slide name or relative path" /></label><StageContinueButton disabled={!selected.length || !canContinue || locked} onClick={onContinue}>Continue with {selected.length} selected</StageContinueButton><button className="btn btn-secondary" disabled={!selected.length || locked} onClick={() => onSelection([])}>Clear selection</button></div>
