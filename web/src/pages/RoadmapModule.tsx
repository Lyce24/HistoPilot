...
const outputs: Record<string, string[]> = {
  ...
  'post-development': ['A completed candidate selected from a specific experiment', 'Pinned fold checkpoints and prediction requirements', 'One frozen predictor per promoted experiment'],
...
export default function RoadmapModule({ module, roadmap, workspace }: { module: Module; roadmap: Roadmap; workspace: Workspace }) {
  ...
  return <div className="clinical-workspace roadmap-module-detail">
    ...
    <section className="module-gate">
      ...
      <div><h2>{module.unlocked ? 'Review this workflow' : 'Complete the prerequisites to unlock this module'}</h2><p>{module.id === 'evaluation' ? 'Inference requires a frozen predictor from Build predictors and a verified test cohort. Test-cohort preparation can proceed while models are still developing.' : module.id === 'post-development' ? 'Complete a development batch before choosing and freezing a predictor. Finished fold runs alone do not publish a predictor.' : 'Your saved work stays available. Complete the required inputs below to continue.'}</p></div>
    </section>
