...
export default function LocalModelEvaluation({ workspace }: { workspace: Workspace }) {
  ...
  return <EvaluationWorkspace key={`${workspace.project.id}:${linkedPredictor}:${linkedCohort}:${linkedEvaluation}`} workspace={workspace} linkedPredictor={linkedPredictor} linkedCohort={linkedCohort} linkedEvaluation={linkedEvaluation} />;
}

function EvaluationWorkspace({ workspace, linkedPredictor, linkedCohort, linkedEvaluation }: { workspace: Workspace; linkedPredictor: string; linkedCohort: string; linkedEvaluation: string }) {
  ...
  const canReview = predictor && cohortReady && name.trim() && !registry.isError && !cohorts.isError;
  const visible = (records.data?.items ?? []).filter((item) => mode !== 'single' || !chosenPredictor || item.manifest.predictorId === chosenPredictor);
  ...
  return <div className="clinical-workspace model-chains">
    <PageHeader eyebrow="03 EVALUATE" title="Evaluate models" description="Choose a predictor and test cohort for each evaluation. Keep every evaluation linked to its source experiment and frozen weights." actions={<div className="inline-actions"><button className="btn btn-primary" onClick={() => { setMode('batch'); setPredictorId(''); publication.reset(); }}>Run all predictors</button><a className="btn btn-secondary" href="#test-data">Test cohorts</a></div>} />
    <EvidenceChain current="evaluation" experimentId={predictor?.manifest.experimentId} predictorId={chosenPredictor} evaluationId={selectedRecord} />
    ...
    {mode === 'batch' ? <Panel title="Run predictors on a test cohort" subtitle="Choose one test cohort and evaluate every compatible ensemble and refit across experiments and seeds."><BulkEvaluationRunner project={project} predictors={registry.data?.items ?? []} cohorts={cohorts.data?.items ?? []} linkedCohort={linkedCohort} onOpenEvaluation={(id) => { setSelectedRecord(id); void records.refetch(); }} /></Panel> : null}
    {mode === 'single' ? <Panel title="Select evaluation inputs" subtitle="Each plan uses one frozen predictor and one test cohort. The same predictor can have several evaluations.">
      <fieldset className="chain-fields" disabled={publication.locked} onChange={() => publication.reset()}>
        ...
        <label className="label">Predictor<select className="field" value={chosenPredictor} onChange={(event) => { setPredictorId(event.target.value); setCohortId(''); setSelectedRecord(''); }}><option value="">Choose a frozen predictor</option>{(registry.data?.items ?? []).filter((item) => item.lifecycleState === 'active' || (item.id === chosenPredictor && item.lifecycleState === 'archived')).map((item) => <option key={item.id} value={item.id}>{item.manifest.name} · {predictorMethodLabel(item.manifest.method)}{item.lifecycleState === 'archived' ? ' · archived' : ''}</option>)}</select></label>
      ...
      {predictor ? <p className="muted">Source: <a href={`#experiments?experiment=${encodeURIComponent(predictor.manifest.experimentId)}`}>{predictor.manifest.experiment?.name ?? shortRecordId(predictor.manifest.experimentId)}</a> · {predictorMethodLabel(predictor.manifest.method)} · {predictor.manifest.checkpoints.length} checkpoints · class target {predictor.manifest.target.field}</p> : <p><a href="#post-development">Build predictors</a> from completed experiment evidence first.</p>}
    ...
    <Panel title="Evaluations and results" subtitle="Compare results within the same test cohort. Each row retains its predictor, seed, method and inputs.">
