...
import { bulkEvaluations } from '../api/bulkEvaluations';
...
function EvaluationWorkspace({ workspace, linkedPredictor, linkedCohort, linkedEvaluation, linkedExperiment }: { workspace: Workspace; linkedPredictor: string; linkedCohort: string; linkedEvaluation: string; linkedExperiment: string }) {
  ...
  const batches = useQuery({ queryKey: ['evaluation-batches', project], queryFn: () => bulkEvaluations.list(project, true), refetchInterval: 5000 });
  ...
  return <div className="clinical-workspace model-chains">
    <PageHeader eyebrow="03 EVALUATE" title={view === 'library' ? 'Model evaluations' : view === 'detail' ? detail?.manifest.name ?? 'Evaluation results' : view === 'batch' ? 'Evaluation batch' : view === 'comparison' ? 'Compare evaluation methods' : 'Evaluate models'} description={view === 'library' ? 'Open evaluation results or create an evaluation for your models.' : 'Select development models and a frozen test cohort, review compatibility, then run evaluation.'} actions={<div className="inline-actions">{view === 'library' ? <button className="btn btn-primary" onClick={create}>Create evaluation</button> : <button className="btn btn-secondary" disabled={locked} onClick={openLibrary}>Back to evaluations</button>}{view !== 'library' ? <a className="btn btn-secondary" href="#test-data">Test cohorts</a> : null}</div>} />
    ...
    {!publication.review ? <Panel title="Select evaluation inputs" subtitle="Each plan uses one frozen predictor and one test cohort. The same predictor can have several evaluations.">
      ...
      <button type="button" className="btn btn-secondary" disabled={publication.locked || !canReview} onClick={() => { if (canReview && reportEditorValidity(editor.current)) void publication.preview({ predictorId: chosenPredictor, cohortId, name: name.trim(), ...evaluationExecutionSelection(inputs) }); }}>Review evaluation</button>
    </Panel> : null}
    {publication.review ? <Panel title="Review evaluation inputs">
      ...
      <div className="stage-actions"><button type="button" className="btn btn-secondary" disabled={publication.locked} onClick={publication.reset}>Back to evaluation inputs</button></div>
    ...
    {view === 'library' ? <StageLibrary project={project} title="Saved evaluations">
      ...
      {libraryTab === 'records' ? <EvaluationResultsTable view="records" records={visible} predictors={registry.data?.items ?? []} experiments={experimentRegistry.data?.items ?? []} cohorts={cohorts.data?.items ?? []} loading={records.isPending} onOpen={openRecord} filters={recordFilters} onFiltersChange={setRecordFilters} actions={libraryActions} /> : <>
    ...
    {view === 'comparison' ? <Panel title="Ensemble and refit comparison" subtitle="Compare matched results on the same test cohort and scoring unit."><EvaluationResultsTable view="comparison" filters={recordFilters} onFiltersChange={setRecordFilters} records={visible} predictors={registry.data?.items ?? []} experiments={experimentRegistry.data?.items ?? []} cohorts={cohorts.data?.items ?? []} loading={records.isPending} onOpen={openRecord} /></Panel> : null}
    {view === 'detail' ? <>{detail ? <EvaluationDetail key={detail.id} project={project} record={detail} /> : <Panel title="Evaluation results"><p role="status">{records.isPending ? 'Loading evaluation…' : records.isError ? 'The evaluation could not be loaded. Retry to check this record.' : 'This evaluation is unavailable. Return to the evaluation library to choose a saved record.'}</p>{records.isError ? <button className="btn btn-secondary" onClick={() => void records.refetch()}>Retry loading evaluation</button> : null}</Panel>}</> : null}
